...
import com.example.dating.Cards.cards;
import com.example.dating.Chat.SendNotification;
...
import com.example.dating.Cards.arrayAdapter;
...
public class MainActivity extends AppCompatActivity {
    ...
    private com.example.dating.Cards.arrayAdapter arrayAdapter;
    private int i;
    private  String tag;
    ...
    private void showToolTip_profile(ShowCasePosition position) {
        new ShowCaseView.Builder(MainActivity.this)
                ...
                .withContent("First time upload your profile picture and click on Confirm wise your app is not working fine")
                .build()
    ...
    private void isConnectionMatch(final String userId) {
        DatabaseReference currentUserNameDb = usersDb.child(currentUId).child("name");
        currentUserNameDb.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                currentUserConnectionsDb.addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists() && !currentUId.equals(userId)) {
                            // Tạo một ChatId mới cho kết nối mới
                            String key = FirebaseDatabase.getInstance().getReference().child("Chat").push().getKey();
                            long now = System.currentTimeMillis();
                            String timeStamp = Long.toString(now);
                            Map<String, Object> mapLastTimeStamp = new HashMap<>();
                            ...
                            usersDb.child(userId).child("connections").child("matches").child(currentUId).updateChildren(mapLastTimeStamp);

                            usersDb.child(currentUId).child("connections").child("matches").child(userId).child("ChatId").setValue(key);
                            usersDb.child(currentUId).child("connections").child("matches").child(userId).updateChildren(mapLastTimeStamp);

                            // Gửi thông báo về kết nối mới
                            DatabaseReference notificationID = usersDb.child(userId).child("notificationKey");
                            notificationID.addListenerForSingleValueEvent(new ValueEventListener() {
                                ...
                                public void onDataChange(@NonNull DataSnapshot snapshot) {
                                    if (snapshot.exists()) {
                                        ...
                                        new SendNotification("You have a new match!", "", notificationKey, null, null);
                                    }
                                }
    ...
    private String userNeed, userGive, userGender;
    ...
    private void checkUserSex() {
        ...
        userDb.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                if (dataSnapshot.exists()){
                    if (dataSnapshot.child("need").getValue() != null && dataSnapshot.child("give").getValue() != null
                    && dataSnapshot.child("gender").getValue() != null){
                        ...
                        getOppositeSexUsers(oppositeUserGive, oppositeUserNeed, userGender);
                    }
                }
            }
...
//                                profileImageUrl, dataSnapshot.child("need").getValue().toString(),
//                                dataSnapshot.child("give").getValue().toString(),  dataSnapshot.child("budget").getValue().toString());
//                        rowItems.add(item);
//                        arrayAdapter.notifyDataSetChanged();
//                    }
...
//    }
    ...
    public void getOppositeSexUsers(final String oppositeUserGive, final String oppositeUserNeed, final String userGender) {
        usersDb.addChildEventListener(new ChildEventListener() {
            @Override
            public void onChildAdded(DataSnapshot dataSnapshot, String s) {
                if (dataSnapshot.exists() && !dataSnapshot.getKey().equals(currentUId)) {
                    String oppositeGender = userGender.equals("Male") ? "Female" : "Male";

                    if (dataSnapshot.child("gender").exists() && dataSnapshot.child("gender").getValue().toString().equals(oppositeGender)) {
                        if (dataSnapshot.child("give").exists() && dataSnapshot.child("need").exists()
                                ...
                                && dataSnapshot.child("give").getValue().toString().equals(oppositeUserGive)
                                && dataSnapshot.child("need").getValue().toString().equals(oppositeUserNeed)) {
                        ...
                        else if (dataSnapshot.child("give").exists() && !dataSnapshot.child("connections").child("nope").hasChild(currentUId)
                                && !dataSnapshot.child("connections").child("yeps").hasChild(currentUId)
                                && dataSnapshot.child("give").getValue().toString().equals(oppositeUserGive)) {
                        ...
                        else if (dataSnapshot.child("need").exists() && !dataSnapshot.child("connections").child("nope").hasChild(currentUId)
                                && !dataSnapshot.child("connections").child("yeps").hasChild(currentUId)
                                && dataSnapshot.child("need").getValue().toString().equals(oppositeUserNeed)) {
                ...
                TextView tv = findViewById(R.id.noCardsBanner);
                if (rowItems.size() == 0) {
                    tv.setVisibility(View.VISIBLE);
                } else {
                    tv.setVisibility(View.INVISIBLE);
    ...
    private void setupTopNavigationView(){
        ...
        TopNavViewHelper.enableNav(MainActivity.this, tvEx);
